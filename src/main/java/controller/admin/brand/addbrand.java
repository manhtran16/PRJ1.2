...
public class addbrand extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        request.getRequestDispatcher("/admin/brand/add_brand.jsp").forward(request, response);
    } 
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            boolean success = brandService.addBrand(brandName);
            if (success) {
                request.setAttribute("success", "Thêm thương hiệu thành công!");
            } else {
        ...
        User user = (User) request.getSession().getAttribute("user");
        request.setAttribute("user", user);
        request.getRequestDispatcher("/admin/add_brand.jsp").forward(request, response);
    }
